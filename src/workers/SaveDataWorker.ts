...
import {TokenContractService, TransactionService, TransferEventService} from '../services';
...
export default class SaveDataWorker {
  ...
  async saveData(message: string) {
    try {
      ...
      //save transfer event
      let transactionHash = '';
      const res = await Promise.all(data.transferEvents.map(async (transferEvent: any) => {
        try {
          ...
          toSaveTransferEvent.amount = BigNumber.from(transferEvent.data).toString();
          const res = await this._transferEventService.save(toSaveTransferEvent);
          const transaction = await this._provider.getTransaction(transferEvent.transactionHash);
